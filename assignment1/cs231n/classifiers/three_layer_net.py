...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        reg_loss1 = np.sum(np.square(W1))
        reg_loss2 = np.sum(np.square(W2))
        reg_loss3 = np.sum(np.square(W3))

        loss = data_loss + 0.5 * reg * (reg_loss1 + reg_loss2 + reg_loss3)
        ...
        # dL/db2 = dL/dscores * dscores/db2 = 1 * dscores
        grads['W3'] = np.dot(hidden_layer_2.T, dscores)
        grads['b3'] = np.sum(dscores, axis=0)

        # dL/dhidden2 = dL/dscores * dscores/dhidden2
        dhidden2 = np.dot(dscores, W3.T)
        dhidden2[hidden_layer_2 <= 0] = 0

        grads['W2'] = np.dot(hidden_layer_1.T, dhidden2)
